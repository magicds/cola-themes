...
(function(win, $, CDSWYDA) {
     ...
     * @param {Boolean} noAdd 不要添加到页面 默认添加
     * @param {Boolean} noCache 不要缓存 默认会缓存 再次加载时直接返回加载好的promise
     */
    CDSWYDA.loadJsPromise = function(url, noAdd, noCache) {
        ...
        script.src = url;
        // append to head
        !noAdd && document.getElementsByTagName('head')[0].appendChild(script);
    ...
    CDSWYDA.loadDisqus = function(el, disqusCfg) {
        ...
        this.loadJsPromise(disqusCfg.jsUrl + '?_t=' + +new Date(), true, true).done(function() {
            disqusContainer.style.display = 'none';
